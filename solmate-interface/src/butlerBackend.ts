...
const CONFIG = {
  ...
  aaveV3Pool: '0x794a61358D6845594F94dc1DB02A252b5b4814aD' as Address, // VERIFIED - Aave V3 Avalanche (via PoolAddressesProvider.getPool)
  aUSDCn: '0x625E7708f30cA75bfd92586e17077590C60eb4cD' as Address, // VERIFIED - Aave V3 aUSDC (native) via getReserveData
  benqiQiUSDCn: '0xB715808a78F6041E46d61Cb123C9B4A27056AE9C' as Address, // VERIFIED - qiUSDCn (native USDC)
  ...
  nativeUSDC: '0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E' as Address,
...
export interface RateData {
  ...
  market: string;
  apy: number; // Percentage (e.g., 5.25 for 5.25%)
...
export interface RebalanceProposal {
  fromProtocol: string;
  toProtocol: string;
  apyDelta: number; // Percentage difference
  estMonthlyGain: number; // Estimated monthly gain in USDC
  estGasCost: number; // Estimated gas cost in USD
  // Split decision (Phase 2.2): the cost buffer is an explicit input, not
  // hidden inside the decision function.
  gasBufferUSD: number; // The pad used to decide wouldExecute
  wouldExecute: boolean; // periodGain > gas + buffer  (Butler says yes / Confirm enabled)
  wouldBreakEven: boolean; // periodGain > gas only (no pad)
  breakEvenDays: number; // days until Δ gain covers gas (ignoring offer)
  periodGain: number; // Δ × principal × (daysHeld/365) in USD
}
...
// ============================================================================
// WALLET & POSITION DATA (Phase 2.1)
// ============================================================================
...
export interface RebalanceIntent {
  id: string; // deterministic hash of the intent so chat confirm matches exactly
  fromProtocol: string;
  toProtocol: string;
  amount: number; // USDC
  apyDelta: number; // percentage points
  estMonthlyGain: number; // USDC (30d)
  estGasUSD: number;
  status: 'preview';
  createdAt: number;
  steps: SimulationStep[];
...
export async function getWalletBalances(
  ...
  const chain = useTestnet ? avalancheFuji : avalanche;
  ...
  try {
    ...
    let currentVenue: 'aave' | 'benqi' | null = null;
 ...
 */
export async function fetchRates(useTestnet: boolean = false): Promise<RateData[]> {
  const chain = useTestnet ? avalancheFuji : avalanche;
  const rpcUrl = useTestnet ? CONFIG.fujiRpc : CONFIG.mainnetRpc;
  ...
  try {
    ...
    if (aaveRate) rates.push(aaveRate);
  } catch (error) {
  ...
  try {
    ...
    if (!useTestnet) {
      ...
      if (benqiRate) rates.push(benqiRate);
    } else {
 ...
 */
function aaveRayToApy(rateRay: number): number {
 ...
 */
function benqiPerSecToApy(ratePerTimestamp: number): number {
...
async function fetchAaveRate(
  ...
  try {
    const reserve = await publicClient.readContract({
      ...
      { data: bigint }, // configuration bitmap
      bigint, // liquidityIndex
      bigint, // currentLiquidityRate (ray)
      bigint, // variableBorrowIndex
      bigint, // currentVariableBorrowRate
      bigint, // currentStableBorrowRate
      number, // lastUpdateTimestamp
      number, // id
      string, // aTokenAddress
      string, // stableDebtTokenAddress
      string, // variableDebtTokenAddress
      string, // interestRateStrategyAddress
 ...
 */
async function fetchAaveDefiLlama(): Promise<RateData | null> {
  try {
    const response = await fetch(CONFIG.defiLlamaAPI);
    const data = await response.json();
    const aavePool = data.data?.find((pool: any) =>
      pool.chain === 'Avalanche' &&
      pool.project === 'aave-v3' &&
      pool.symbol === 'USDC'
    );
    if (!aavePool) return null;
    return {
      protocol: 'aave',
      market: 'USDC',
      apy: aavePool.apy || 0,
 ...
 */
async function fetchBenqiDefiLlama(): Promise<RateData | null> {
  try {
    const response = await fetch(CONFIG.defiLlamaAPI);
    const data = await response.json();
    const benqiPool = data.data?.find((pool: any) =>
      pool.chain === 'Avalanche' &&
      pool.project === 'benqi-lending' &&
      pool.symbol === 'USDC'
    );
    if (!benqiPool) return null;
    return {
      protocol: 'benqi',
      market: 'qiUSDCn',
      apy: benqiPool.apy || 0,
...
export function decideRebalance(
  currentPosition: { protocol: string; amount: number },
  rates: RateData[],
  riskTier: 'conservative' | 'moderate' | 'aggressive' = 'moderate',
  daysHeld: number = CONFIG.daysHeldDefault,
  gasCostUSD: number = 0.02
...
export async function previewRebalance(
  currentPosition: { protocol: string; amount: number },
  useTestnet: boolean = false,
  userAddress: `0x${string}` = '0x0000000000000000000000000000000000000000',
  riskTier: 'conservative' | 'moderate' | 'aggressive' = 'moderate'
): Promise<{ proposal: RebalanceProposal | null; rates: RateData[]; intent: RebalanceIntent | null }> {
...
export async function executeRebalance(
  ...
  wallet: any // Viem WalletClient type
